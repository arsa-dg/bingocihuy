#ngubah -> eror -> print("kapokmu bingung dewe")
import random   
import os       
import time     
...
#level bot hard
#multiplayer
#kalo user gamasukin angka di awal eror
...
def Bingo_Player_Turn():
    ...
    while(not(num_Valid)):
        try:
            turn = int(input("Masukkan angka: "))
        except ValueError:
            print("Masukkan angka!")
        if (1 <= turn <= 25 or turn == 999):
            num_Valid = True
        else:
...
while (Bingo_Play == "Y"):
    ...
    time.sleep(1)
    Bingo_Level = input("Level?(EZ/Nanggung) ")
    #misstype
    if (Bingo_Level == "ez" or Bingo_Level == "Ez"):
        Bingo_Level = "EZ"
    elif (Bingo_Level == "nanggung" or Bingo_Level == "NANGGUNG"):
        Bingo_Level = "Nanggung"
    ...
    time.sleep(1)
    Bingo_Mode = input("Mode?(Casual/Pro) ")
    ...
    #input play
    time.sleep(1)
    Bingo_Play = input("Main lagi?(Y/N) ")
    while not (Bingo_Play == "Y" or Bingo_Play == "N"):
        print("Masukkan (Y/N)!")
        time.sleep(2)
        Bingo_Play = input("Main lagi?(Y/N) ")
